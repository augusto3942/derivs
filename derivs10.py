...
st.markdown("""
<style>
/* Página inteira escura */
.stApp, .css-18e3th9, .reportview-container { background-color: #03045e !important; color: #f0f4f8 !important; }
/* Container principal transparente para herdar fundo */
.block-container { background-color: transparent !important; }
/* Sidebar escura */
[data-testid="stSidebar"] { background-color: #02215a !important; color: #f0f4f8 !important; }
/* Botões cor */
.stButton>button { background-color: #1f77b4 !important; color: white !important; }
/* Inputs e selects */
.css-1lcbmhc, .css-1avcm0n, .css-1jcwzny, .css-14xtw13 { background-color: #021f59 !important; color: #f0f4f8 !important; }
/* Títulos e texto */
h1, h2, .css-18e3th9 h1, .css-18e3th9 h2, .css-1d391kg { color: #f0f4f8 !important; }
/* Slider */
.css-1kyxreq .css-1emrehy .stMarkdown { color: #f0f4f8 !important; }
</style>
...
def plot_trajetorias(S0, T, r, sigma, n_sim, m=50) -> plt.Figure:
    dt = T / m
...
ticker = st.sidebar.selectbox("Ticker", TICKER_SUGGESTIONS)
...
if st.button("Calcular"):
    try:
        ...
        st.success(f"Preço da Opção {style}: {preco:.4f}")
...
st.markdown("---")
st.subheader("Volatilidade Implícita")
...
if st.button("Calcular IV"):
    try:
        ...
        for _ in range(50):
            mid = (lo + hi) / 2
            diff = monte_carlo_opcao_europeia(S0_input, K, T, r, mid, n_sim) - market_price
            if diff > 0:
                hi = mid
            else:
                lo = mid
