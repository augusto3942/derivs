...
def binomial_americana_call(S0, K, T, r, sigma, n_steps):
    dt = T/n_steps
    up = np.exp(sigma*np.sqrt(dt)); down = 1/up
...
def plot_trajetorias(S0, T, r, sigma, n_sim, m=50):
    dt = T/m
    paths = np.zeros((m+1, n_sim)); paths[0] = S0
    for t in range(1, m+1):
        Z = np.random.standard_normal(n_sim)
        paths[t] = paths[t-1]*np.exp((r-0.5*sigma**2)*dt + sigma*np.sqrt(dt)*Z)
    fig, ax = plt.subplots()
    ax.plot(paths[:, :min(10,n_sim)], alpha=0.6)
    ax.set_xlabel('Passo'); ax.set_ylabel('Preço'); ax.set_title('Trajetórias MC')
    return fig
...
with col1:
    ...
    if style=="Americana": n_steps = st.number_input("Passos Binomial",10,1000,100)
with col2:
    T = st.number_input("Tempo (anos)",1.0,0.01,1.0,0.01)
    r = st.number_input("r (juros)",0.04,format="%.4f")
    sigma_input = st.number_input("σ",sigma_hist if 'sigma_hist' in locals() else 0.2,format="%.4f")

n_sim = st.slider("Simulações",1000,100000,20000,1000)

if st.button("Calcular"):
    try:
        if style=="Europeia": preco = monte_carlo_opcao_europeia(S0_input,K,T,r,sigma_input,n_sim)
        elif style=="Asiática": preco = monte_carlo_opcao_asiatica(S0_input,K,T,r,sigma_input,n_sim)
        else: preco = binomial_americana_call(S0_input,K,T,r,sigma_input,n_steps)
...
mp = st.number_input("Preço de Mercado",0.0)
if st.button("Calcular IV"):
    try:
        f = lambda sig: monte_carlo_opcao_europeia(S0_input,K,T,r,sig,n_sim)-mp
        lo,hi=1e-4,5.0
        for _ in range(50): mid=(lo+hi)/2; (hi if f(mid)>0 else lo)=mid
        st.write(f"IV: {(lo+hi)/2:.2%}")
    except Exception as e:
        st.error(f"Erro: {e}")
